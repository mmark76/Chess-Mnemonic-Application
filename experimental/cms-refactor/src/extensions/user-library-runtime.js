// Experimental CMS — user-library runtime (pipeline hooks; no renderAll monkey-patch race).
(function () {
  ...
  function defaultHideTargetSquareColumns() {
    ["assocSection", "shortnamesSection"].forEach(function (sectionId) {
  ...
  function defaultHideAnchorColorAndActionColumns() {
    ["sanSection", "assocSection", "shortnamesSection", "pao99Section", "paoSection"].forEach(function (sectionId) {
      var section = document.getElementById(sectionId);
      if (!section) return;
      var table = section.querySelector("table");
      if (!table) return;
  ...
  if (typeof CMA_RENDER_PIPELINE !== "undefined") {
    CMA_RENDER_PIPELINE.register("user-library-runtime", function () {
      applyDefaultFullMoveMode();
    }, function () {
